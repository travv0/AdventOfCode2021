...
let walkNodes (from: string) (to_: string) part connections : list<list<string>> =
    let rec walkNodes'
        from
        ...
        if from = to_ then
            ...
            | Some children ->
                ...
                children
                |> Seq.map (fun c -> walkNodes' c newDeadNodes (from :: path) newTwoTimeQueue)
                |> Seq.concat
                |> Seq.toList
    ...
    | Part2 ->
        let smallCaves =
            ...
            |> Seq.filter (fun cave -> isSmallCave cave && cave <> from && cave <> to_)
...
|> List.length
|> printfn "There are %d paths through the cave system if each small cave is visited once."

connections
|> walkNodes "start" "end" Part2
|> List.length
|> printfn "There are %d paths through the cave system if one small cave can be visited twice."
